...
```
curl -sS https://packages.gitlab.com/install/repositories/gitlab/gitlab-ce/script.rpm.sh | sudo bash
yum install gitlab-ce-9.4.5* -y --nogpgchec # 安装9.4.3版本的gitlab
```
...
```
yum install gitlab-ce-9.4.5* -y --nogpgchec # 加上--nogpgchec
```
...
这个git project莫名其妙坏掉了，服务器上对应的lfs object应该是损坏了。
...
需要从头至尾完整安装gitlab（因为部分服务未安装或者未启动）
