...
```
curl -sS https://packages.gitlab.com/install/repositories/gitlab/gitlab-ce/script.rpm.sh | sudo bash
yum install gitlab-ce-10.8.3* -y --nogpgchec # 安装10.8.3版本的gitlab
```
...
```
yum install gitlab-ce-10.8.3* -y --nogpgchec # 加上--nogpgchec
```
...
```
Solution:
使用命令行会报错,使用gitlab自带nginx启动(重启服务器后)，直接使用SourceTree提交。
